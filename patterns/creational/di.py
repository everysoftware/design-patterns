...
dependencies itself, but receives them from the outside, which is often implemented through DI. IoC allows for reduced
coupling between components and simplifies testing and replacing individual parts of the system.

Dependency Injection (DI) is a creational design pattern (technique) that allows an object to receive its dependencies
from an external source rather than creating them itself.

DI Container is a class that creates a single place that is responsible for creating and managing the dependencies
of an object.
"""
